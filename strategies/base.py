...
class StrategyBase(bt.Strategy):
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                self.log('已买入, 价格: %.2f,总价:%.2f  佣金 %.2f 持仓 %.2f 持仓价格: %.2f' %
                ...
                self.log('已卖出, 价格: %.2f,总价:%.2f  佣金 %.2f 持仓 %.2f 持仓价格: %.2f' %
                         (order.executed.price,
                          order.executed.value,
                          order.executed.comm,
                          self.position.size,
    ...
    def notify_trade(self, trade):
        ...
        msg =  colored('OPERATION PROFIT, GROSS %.2f, NET %.2f' % (trade.pnl, trade.pnlcomm), color)
        self.log(msg)
        text.writeline("profit",msg)
